  ...
  xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        obj.id = ss[0]; obj.unit_code = ss[1]; obj.lecturer = ss[2]; obj.venue = ss[3]; obj.timeshift = ss[4]; obj.day = ss[5];
      ...
      final_lectures.forEach(element => {
        ...
        row += "</td><td>"+element.constraint.const_day + " " + element.constraint.const_time + " " + element.constraint.const_venue;
        row += "</td></tr>";
